...
CLONE_DIR="${DEVELOPER_ID}-${TESTING_REPO_NAME}_PyMOP_12"
...
/usr/bin/time -v timeout -k 9 3400 bash -c 'PYMOP_SPEC_FOLDER="$PWD"/../../Specs/PyMOP PYMOP_ALGO=D PYMOP_STATISTICS=yes PYMOP_STATISTICS_FILE=D.json PYMOP_INSTRUMENTATION_STRATEGY=ast PYTHONPATH="$PWD"/../mop-with-dynapt/pythonmop/pymop-startup-helper/ pytest --continue-on-collection-errors' &> ${TESTING_REPO_NAME}_Output.txt
exit_code=$?
