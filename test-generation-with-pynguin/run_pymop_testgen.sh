...
CLONE_DIR="${DEVELOPER_ID}-${TESTING_REPO_NAME}_PyMOP_Testgen"
...
timeout -k 9 3000 bash -c 'PYMOP_SPEC_FOLDER="$PWD"/../mop-with-dynapt/specs-new PYMOP_ALGO=D PYMOP_STATISTICS=yes PYMOP_STATISTICS_FILE=D.json PYMOP_INSTRUMENTATION_STRATEGY=ast PYTHONPATH="$PWD"/../mop-with-dynapt/pythonmop/pymop-startup-helper/ pytest --continue-on-collection-errors testgen/' > ${TESTING_REPO_NAME}_Output.txt
exit_code=$?
