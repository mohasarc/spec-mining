...
CLONE_DIR="${DEVELOPER_ID}-${TESTING_REPO_NAME}_Testgen"
...
mkdir -p testgen
cp ../../${TESTING_REPO_NAME}/testgen/* testgen/

# Record test start time
TEST_START_TIME=$(python3 -c 'import time; print(time.time())')

# Run tests with 1-hour timeout and save output
timeout -k 9 3000 pytest --continue-on-collection-errors testgen/ > ${TESTING_REPO_NAME}_Output.txt
exit_code=$?
...
if [ $exit_code -ne 124 ] && [ $exit_code -ne 137 ]; then
    ...
    # Determine which source package to measure
    if [ -d "src" ]; then
        SOURCE_PKGS="src"
    elif [ -d "${TESTING_REPO_NAME}" ]; then
        SOURCE_PKGS="${TESTING_REPO_NAME}"
    else
        SOURCE_PKGS="."
    fi

    # Collect coverage on source code only, not testgen
    echo "Collecting coverage on source code only, not testgen"
    echo "SOURCE_PKGS: $SOURCE_PKGS"
    pytest --continue-on-collection-errors testgen/ \
        --cov="$SOURCE_PKGS" \
        --cov-report=term-missing:skip-covered \
        --cov-report=term \
        --cov-report=xml:${TESTING_REPO_NAME}_Coverage.xml \
        --cov-branch \
        --cov-fail-under=0 \
        > ${TESTING_REPO_NAME}_Coverage.txt 2>&1

    # Show coverage summary
    tail -n 3 ${TESTING_REPO_NAME}_Coverage.txt
    ...
    # Copy coverage output to results directory
